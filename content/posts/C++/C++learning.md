...
4. **当派生类对基类的虚函数没有重写时，派生类的虚表指针指向的是基类的虚表**；**当派生类对基类的虚函数重写时，派生类的虚表指针指向的是自身的虚表**；



> vtable存储在哪个段？

vtable存储在在内存段的只读数据段（.rodata），因为vtable的内容（虚函数地址序列）在编译时就已经确定了，而且vtable的内容不应该被修改（否则破坏多态）且.rodata段具有写保护。
