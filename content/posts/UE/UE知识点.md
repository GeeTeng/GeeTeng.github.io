...
GameMode：游戏玩法模式，取得当前游戏多长时间，13分获胜。

GameState：主要存储所有客户端必须一致的对局级信息，例如比赛阶段、回合状态、双方比分

PlayerState：击杀数/死亡数/助攻数；当前大招点数；金钱等
...
Lyra动画蓝图架构中最主要的技术是**动画蓝图链接**，也就是Lyra 不把动画逻辑和动画资源写死在同一个动画蓝图里，而是通过“动画蓝图链接 + 动画层接口”，把“动画在什么状态下播放”和“具体播哪套动画”拆开。

同时，动画蓝图链接还有一个很重要的特性是：**只有被引用的动画资产才会被加载进入内存当中。**如果动画蓝图链接对各个模块的划分比较合理的话，可以节省很多的内存资源，达到一个动态加载卸载的效果。